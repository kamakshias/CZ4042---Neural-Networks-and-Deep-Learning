...
df=pd.read_csv('/home/UG/atrik001/imdb.csv') #/home/UG/atrik001/imdb.csv
...
df_age_y=[]
for i in range(len(df)):
    image = Image.open('/home/UG/atrik001/imdb_crop/' + str(df['img_paths'][i]))
...
x_train, x_val, x_test = np.split(df_image_x, [int(len(df_image_x)*0.7), int(len(df_image_x)*0.85)])

y_train, y_val, y_test = np.split(df_age_y, [int(len(df_age_y)*0.7), int(len(df_age_y)*0.85)])
...
checkpoint = keras.callbacks.ModelCheckpoint(drive_prefix  + '/pretrained_checkpoint_age.h5', monitor='val_accuracy', verbose=1, mode='max',save_best_only = True)
csv_logger = keras.callbacks.CSVLogger(drive_prefix  + '/pretrained_csvlog_age.csv')
