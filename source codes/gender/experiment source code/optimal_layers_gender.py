...
# # Experimenting with number of hidden dense layers [1,2,3] with:
# ## 1) Number of neurons = 512[optimal]
# ## 2) Batch size = 16 [optimal]
# ## 3) Number of CNN layers as 1
# 
...
def step_decay(epoch):
    ...
    lrate = init_lrate * math.pow(drop, math.floor((1+epoch)/epochs_drop))
    return lrate
sgd = keras.optimizers.SGD(learning_rate,momentum=0.0) 
